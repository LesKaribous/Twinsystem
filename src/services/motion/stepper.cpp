...
void Stepper::control() {
    ...
    int step_delay = 1.0e6 / fabs(m_current_velocity);

    // Update direction
    setDirection(m_current_velocity > 0);

    // Use a proper time-tracking mechanism instead of a static variable
    static unsigned long last_step_time = micros();  // Store last step time
