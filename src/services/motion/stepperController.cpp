...
void StepperController::complete() {
    ...
    m_sA.m_position = m_sB.m_position = m_sC.m_position = 0;
    m_sA.m_start = m_sB.m_start = m_sC.m_start = 0;
    m_sA.m_target = m_sB.m_target = m_sC.m_target = 0;
    m_sA.m_finalTarget = m_sB.m_finalTarget = m_sC.m_finalTarget = 0;
    m_sA.m_totalDistance = m_sB.m_totalDistance = m_sC.m_totalDistance = 0;
    m_totalTime = 0;
    m_startTime = 0;
    Job::complete();
}
...
void StepperController::cancel() {
    ...
    while (fabs(m_leadStepper->getVelocity()) > 1.0f) {
        unsigned long now = micros();

        if (now - lastControlTime >= Settings::Stepper::STEPPER_DELAY) {
            control();
            lastControlTime = now;
        }

        if (now - lastRunTime >= Settings::Stepper::STEPPER_COMPUTE_DELAY) {
            run();
            lastRunTime = now;
        }
    }
    ...
    complete();
    Job::cancel();
    Console::trace("cancel") << "Cancelled and stopped cleanly." << Console::endl;
}
