...
template<typename T>
constexpr std::shared_ptr<T> SystemApplication::loadModule(){
    std::shared_ptr<T> ptr = std::make_shared<T>();
    system.loadModule(ptr);
    return ptr;
}
...
void SystemApplication::update(){
	...
	if(_state == RobotState::STARTED || _state == RobotState::FINISHING){
		...
		if(lidar.obstacleDetected() && motion.isMoving() && !motion.isRotating()){
			motion.pause();
            THROW("PAUSED")
		}else if(motion.isPaused() && !lidar.obstacleDetected()){
		    motion.resume();
            THROW("RESUMED")
		}
	}
...
void SystemApplication::goAlign(Vec2 target, RobotCompass rc, float orientation){
    motion.goAlign(target, rc, orientation);
	waitUntil(motion);
}
